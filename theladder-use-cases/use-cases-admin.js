...
const offlineRequestKey = 'aesop-use-case-training-requests-v1';
...
function renderRequests() {
  ...
  if (!filtered.length) {
    elements.requestList.innerHTML = '<p class="empty-state">No use case requests match this view.</p>';
    return;
  }
  ...
  return `
    <article class="admin-request-card status-${escapeHtml(request.status || 'requested')}">
      <div class="request-card-main">
        ...
        <h3>${escapeHtml(request.useCaseName)}</h3>
        <dl>
          <div>
            <dt>Topic</dt>
            <dd>${escapeHtml(request.topic || 'Unassigned')}</dd>
          </div>
          <div>
            <dt>Requested by</dt>
          ...
          ${request.sourceUseCaseName ? `<div><dt>Submitted from</dt><dd>${escapeHtml(request.sourceUseCaseName)}</dd></div>` : ''}
        </dl>
        ${localNote}
      </div>
